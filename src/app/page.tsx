...
export default function HomePage() {
  ...
  // Auto-scan on mount + periodik refresh (live mode)
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      void runScan();
    }, 120000); // refresh tiap 2 menit pas live mode

    return () => clearInterval(interval);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [runScan, liveMode, scanResults.length]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (key === "1") setActiveTab("scanner");
      else if (key === "2") setActiveTab("analysis");
      else if (key === "3") setActiveTab("comparison");
      else if (key === "4") setActiveTab("timeframe");
      else if (key === "5") setActiveTab("breadth");
      else if (key === "6") setActiveTab("settings");
      else if (key === "r" && !e.ctrlKey && !e.metaKey) runScanRef.current();
    };
  ...
  const runAIDirectly = async () => {
    ...
    try {
      ...
      const result = await callAIIfEnabled({
        system:
          "Kamu adalah asisten trading saham IDX. Berikan second opinion dalam Bahasa Indonesia yang ringkas, langsung ke inti, dan fokus pada risiko + eksekusi. Berikan JAWABAN LANGSUNG tanpa menunjukkan proses berpikir atau perencanaan.",
        user: prompt,
        settings: aiSettings,
      });
      if (!result) {
      ...
      <header className="border-b border-zinc-800/70 bg-zinc-950/90">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {liveMode && <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />}
              LIVE
            </button>
            <ThemeToggle />
        ...
        {activeTab === "scanner" && (
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <aside className="space-y-4">
              ...
              <Card>
                <CardContent className="p-5">
                  ...
                  <div className="mt-4 space-y-3 text-xs">
                    ...
                    <StatusRow label="Aturan" value={scanMode === "day" ? "VWAP/volume/RR 1.2" : scanMode === "swing" ? "EMA20/RR 1.5" : scanMode === "premarket" ? "Gap/trend/RR 1.2" : "RR ketat 2.0"} />
                    <StatusRow
                      label="Scan Terakhir"
                      value={
                        scanCompletedAt
              ...
              <SectorHeatmap scanResults={scanResults} />
            </aside>
          </div>
        )}
